...
public class RobotLED {
  private static int NUM_LEDS = 110; // The number of LEDs on the strip
  private CANdle candle = new CANdle(Constants.CANdleIDConstants.candleID1); // CANdle is the CTRE hardware we use to configure our LEDs
  StrobeAnimation bufferedAnimation = new StrobeAnimation(0, NUM_LEDS);
...
// Sets color and speed
  public void strobeAnimation(RGBWColor color) {
    bufferedAnimation.withColor(color).withFrameRate(50);
  }
// Sets color to CANdle
  public void updateLEDs() {
    candle.setControl(bufferedAnimation);
  }
// Has to be called in GameState's periodic method since this isn't a subsystem
